...
#RecordAddForm .form_list {
  float:left;
  margin-right: 30px;
  margin-bottom:10px;
}

#RecordAddForm #master_category, #RecordAddForm #use_credit_card {
  width: 120px;
}

#RecordAddForm #description {
  width: 250px;
}
...
<?php echo $this->Form->create('Record');?>
  ...
  <div class="form_list">
    <h3>使った人</h3>
    <select name="data[Record][vouncer_id]">
    <?foreach($vouncers as $id => $name):?>
  ...
  <div class="form_list">
    <h3>日付</h3>
    <input type="text" id="recordDate" name="data[Record][date]" value="" />
  </div>
  <br class="clearFix" />

  <div id="master_category" class="form_list">
    <h3>大カテゴリ</h3>
    <select name="data[Record][master_category_id]">
    <?foreach($masterCategories as $id => $name):?>
      <option value="<?=$id?>"><?=$name?></option>
    <?endforeach;?>
    </select>
  </div>
  <div id="info_category" class="form_list">
    <h3>詳細カテゴリ</h3>
    <select name="data[Record][info_category_id]">
  ...
  <div class="form_list">
    <h3>内容</h3>
    <input type="text" id="description" name="data[Record][description]" value="" />
  </div>
  <br class="clearFix" />

  <div class="form_list">
    <h3>金額</h3>
    <input type="number" id="price" name="data[Record][price]" value="" />
  </div>
  <br class="clearFix" />
